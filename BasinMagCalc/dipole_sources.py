...
def calc_all_choclo(
        ...
        location: np.ndarray,
        M_vector: np.ndarray,
        source_type = 'prism') -> np.ndarray: 

    B_tot_out = np.zeros(location.shape)

    if source_type == 'prism':
        for i in numba.prange(len(location)):
            for j in range(len(x_grid)):
                Bx,By,Bz = choclo.prism.magnetic_field(location[i,0], location[i,1], location[i,2], 
                                x_grid[j]-0.5*V_grid[j]**(1/3), x_grid[j]+0.5*V_grid[j]**(1/3), 
                                y_grid[j]-0.5*V_grid[j]**(1/3), y_grid[j]+0.5*V_grid[j]**(1/3), 
                                z_grid[j]-0.5*V_grid[j]**(1/3), z_grid[j]+0.5*V_grid[j]**(1/3), 
                                M_vector[j,0], M_vector[j,1], M_vector[j,2])
                B_tot_out[i,0] += Bx
                B_tot_out[i,1] += By
                B_tot_out[i,2] += Bz
        ...
        for i in numba.prange(len(location)):
            for j in range(len(x_grid)):
                Bx, By, Bz = choclo.dipole.magnetic_field(location[i,0], location[i,1], location[i,2], 
                                x_grid[j], y_grid[j], z_grid[j], 
                                M_vector[j,0]*V_grid[j], M_vector[j,1]*V_grid[j], M_vector[j,2]*V_grid[j])
